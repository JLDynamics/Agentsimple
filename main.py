...
def most_recent_session_name() -> str | None:
    ...
    newest = max(sessions, key=lambda session: session.get("updated_at", ""))
    return newest["name"]
...
def resume_session_picker(system_message: dict) -> tuple[str, list[dict]]:
    ...
    for index, session in enumerate(sessions, start=1):
        ...
        print(
            f"{index}. {name} - {when} "
            f"- {session['message_count']} msgs{preview_text}"
        )
    ...
    while True:
        ...
        loaded = apply_system_message(
            load_session(target_session["name"]), system_message
        )
        print()
...
def choose_session(
    ...
    while True:
        ...
        if keyword in ("delete", "name", "export", "resume"):
            ...
            loaded_messages = apply_system_message(
                load_session(target_session["name"]), messages[0]
            )
            print()
            print(f"Resumed session: {target_session['name']}")
            print()
            return target_session["name"], loaded_messages
...
def build_system_prompt() -> str:
    return (
        ...
        "Actively prune stale memory: when a fact has become outdated, wrong, or no longer relevant, remove it by rewriting the memory without that fact, so the memory stays current and accurate. "
